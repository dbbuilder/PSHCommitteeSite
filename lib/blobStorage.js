...
export async function uploadFileToBlob(file, filename) {
  try {
    if (!process.env.BLOB_READ_WRITE_TOKEN) {
      throw new Error('BLOB_READ_WRITE_TOKEN not configured');
    }

    const blob = await put(`${DOCUMENTS_PREFIX}${filename}`, file, {
      access: 'public',
...
export async function initializeMetadata() {
  try {
    ...
    const response = await head(METADATA_KEY);
    if (!response) {
      // Create initial metadata
      await saveDocumentsMetadata(defaultDocuments);
    }
  } catch (error) {
    // If error (likely 404), create initial metadata
